...
COPY public.accounts (account_id, username, password_hash, password_salt, role_account, must_reset_password, password_last_rotated_at, created_at, last_login_at, is_active) FROM stdin;
\.
...
COPY public.contest (contest_id, name, starts_at, ends_at, scoreboard_visible) FROM stdin;
\.
...
COPY public.credential_issue (credential_issue_id, issued_to_account_id, issued_by_account_id, password_hash, password_salt, issued_at, expires_at, used_at, must_reset, revoked, notes) FROM stdin;
\.
...
COPY public.judge (judge_id, display_name, account_id) FROM stdin;
\.
...
COPY public.judging (judging_id, submission_id, judge_id, started_at, result, notes) FROM stdin;
\.
...
COPY public.problem (problem_id, contest_id, code, title, statement_md, allowed_languages, base_points, is_active) FROM stdin;
\.
...
COPY public.registration (registration_id, contest_id, team_id, registered_at) FROM stdin;
\.
...
COPY public.submission_testcase (stc_id, submission_id, test_case_id, status, runtime_ms, stderr_snip) FROM stdin;
\.
...
COPY public.submissions (submission_id, contest_id, team_id, problem_id, language_code, source_code, recieved_at, judged_at, status, judged_by, feedback) FROM stdin;
\.
...
COPY public.teams (team_id, team_number, team_name, account_id) FROM stdin;
\.
...
COPY public.test_case (test_case_id, problem_id, input_blob, expected_output_blob, timeout_ms, ordinal) FROM stdin;
\.
...
\unrestrict hdFiNeE86hsGECAuxEDcbD9zABYEVhpWifBToMVHR2SDx1sCMu9ruMGUHzlShDh
