const judgeSocket = io(window.location.origin, {
...
function fitJudgeTerminal() {
    if (!judgeTerm.element || !judgeTerm._core || !judgeTerm._core._renderService) {
	return;
    }

    const dims = judgeTerm._core._renderService.dimensions;
    if (!dims || !dims.css || !dims.css.cell.width || !dims.css.cell.height) {
	return;
    }

    const availableWidth = judgeTerm.element.clientWidth;
    const availableHeight = judgeTerm.element.clientHeight;
    ...
    judgeTerm.resize(cols, rows);
    judgeTerm.scrollToBottom();
}

setTimeout(fitJudgeTerminal, 0);
window.addEventListener("resize", fitJudgeTerminal);
