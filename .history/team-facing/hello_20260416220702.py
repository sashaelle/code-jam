...
def compile_button(data):
    ...
    if current_language == ".java":
        ...
        src = os.path.join(base_dir, "submissions", "submission.cpp")
        out = os.path.join(base_dir, "submissions", "submission")

        with open(src, "w", newline="\n") as f:   # <-- write to absolute path
            f.write(code)
        print("src:", src)
        print("src exists:", os.path.exists(src))
        print("out:", out)
        print("submissions dir exists:", os.path.exists(os.path.join(base_dir, "submissions")))
        result = subprocess.run(
    f'"{r"C:\MinGW\bin\g++.exe"}" "{src}" -o "{out}"',
    shell=True,
    stdout=subprocess.PIPE,
    stderr=subprocess.PIPE,
    text=True,
...
print("return code:", result.returncode)
        ...
        process = subprocess.Popen([out + ".exe"],
                                   stdin=subprocess.PIPE,
                                   stdout=subprocess.PIPE,
                                   stderr=subprocess.PIPE,
                                   text=True,
