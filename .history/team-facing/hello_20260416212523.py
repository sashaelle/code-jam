...
def compile_button(data):
    ...
    if current_language == ".java":
        ...
        fp_abs = os.path.abspath(fp)  # make fp absolute
        output = os.path.join(script_dir, "submissions", "submission")
        exe = os.path.join(script_dir, "submissions", "submission.exe")

        compile_result = subprocess.run(
            ["C:\\MinGW\\bin\\g++.exe", fp_abs, "-o", output],
            capture_output=True,
            text=True,
            cwd=script_dir
        )
